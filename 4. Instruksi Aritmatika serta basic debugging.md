...
- 8 bit, membagikan `ax` dengan sumber, menyimpan hasilnya di `al`, sisa pembagian di `ah`.
- 16 bit, membagikan `dx:ax` 32 bit , menyimpan hasil di `ax`, sisanya  di `dx`.
- 32 bit, membagikan `edx:eax` 64 bit, menyimpan hasil di `eax`, sisanya di `edx`.
- 64 bit, membagikan `rdx:rax` 128 bit, menyimpan hasil di `rax`, sisanya di `rdx`.
...
mov edi, 10
idiv edi; edx = eax:edx % eax
        ; eax = eax:edx / edi
```
Nilai `eax` adalah 0, dan `edx` adalah 0.
...
xor edx, edx ; edx = 0
mov edi, 10
div edi; edx = eax:edx % eax
        ; eax = eax:edx / edi  
```
Maka nilai __eax__ akan menjadi 0, dan nilai __edx__ menjadi 10.
...
```asm
xor eax, eax  ;set eax menjadi 0
inc eax     ;eax +1
inc eax     ;eax +1
```
Maka hasilnya adalah 2.
```asm
mov eax, 10 ;set eax menjadi 10
dec eax     ;eax -1
dec eax     ;eax -1
```
