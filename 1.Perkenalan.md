...
_start:
        ...
        syscall
```
![gambar](https://user-images.githubusercontent.com/86765295/173207543-4752d580-0ae4-458d-8537-6b1f7e70ece7.png)
...
`section .data` bersifat *read write*.
...
Seperti namanya, `section .rodata` ini *read only*.
...
`%define` merupakan macro, seperti `#define` dalam bahasa C/C++  

`%define WRITE 1` sama seperti `#define WRITE 1`

begitpun yang lainnya.
...
``` 
`eax = WRITE`
Memasukkan nilai 1 / `WRITE` ke dalam **eax**.
...
`edi = STDOUT_FILENO` Memasukkan nomor 1 / `STDOUT_FILENO` ke dalam **edi**.
...
`esi = msg` Mengisi **esi** dengan nilai **msg**.
...
mov edx, 14
```
`edx = 14` Mengisi **edx** dengan nilai 14.
...
mov eax, 60
```
`eax = EXIT` Mengisi **eax** dengan nilai `EXIT` / 60.
...
xor edi, edi
```
`edi ^= edi` Membuat register **edi**  bernilai 0 dengan instruksi bitwise `xor`.
