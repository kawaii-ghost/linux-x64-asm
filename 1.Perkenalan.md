...
Jika dibandingkan dalam fungsi C/C++, maka kode di atas akan sama seperti
...
Operator `mov` digunakan untuk mengisi suatu data.
...
mov eax, 1
``` 
berarti memasukkan nilai **1** ke dalam register eax.
...
Register **rdi** adalah register yang digunakan untuk memberikan argumen pertama ke fungsi atau system call.
...
`stdin` untuk menerima input, `stdout` menampilkan output, dan `stderr` untuk menampilkan pesan diagnosa atau error.
...
Karena kita ingin menampilkan pesan ke `stdout` maka kita mengisi register **rdi** dengan nilai 1
...
mengisi nilai alamat `msg` ke dalam register **rsi**.
...
Ukuran `msg` tanpa karakter NULL adalah 14.

Kode-kode di atas tadi menghasilkan
...
Instruksi `syscall` digunakan untuk memanggil kernel agar melakukan system call.
...
Mengisi nilai 60 ke register **eax**
...
Membuat 0 register **edi** dengan instruksi bitwise `xor`.

`xor` digunakan untuk mengisinya dengan nilai 0 karena lebih ringan dibanding instruksi `mov`.

Jika dituang dalam bahasa C/C++ `edi ^= edi`.
