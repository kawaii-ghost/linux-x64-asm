...
``` 
`rax = 1`
Memasukkan nilai 1 / `WRITE` ke dalam **rax**.
...
`rdi = 1` Memasukkan nomor 1 ke dalam **rdi**.
...
mov rax, 60
```
`rax = 60` Mengisi **rax** dengan nilai 60.
...
mov rdi, 0
```
`rdi = 0` Mengisi register **rdi**  dengan nilai 0.
...
Compiler mengoptimisasi kode assemblynya, kita akan membahas alasannya seiring bertambahnya materi yang kita pelajari.
